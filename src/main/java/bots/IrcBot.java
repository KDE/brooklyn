...
public final class IrcBot implements Bot {
    ...
    private final Map<String, String> webserverConfig = new HashMap<>(0);
    ...
    public void onJoin(ChannelUserListChangeEvent event) {
        ...
        if (!authorNickname.equals(client.getNick())) {
            ...
            if (0 == change.compareTo(ChannelUserListChangeEvent.Change.JOIN))
                ...
                if (channelFrom.isPresent())
                    message = MessageFormat.format(resourceBundle.getString("channel-leaved"), authorNickname);
                else
                    message = MessageFormat.format(resourceBundle.getString("server-leaved"), authorNickname);
            }
